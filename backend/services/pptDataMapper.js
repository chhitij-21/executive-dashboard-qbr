...
function mapSnapshotToPPTData(qbrData) {
  ...
  const pptData = {
    ...
    slide3_ExecutiveSummary: {
      ...
      slaTarget: { value: exec.slaTarget || 99.3, path: 'executiveSummary.slaTarget' },
    ...
    slide5_SiteOverview: {
      sites: sites.map((s, idx) => ({
        ...
        slaStatus: parseFloat(s.jflSwitchUptime || s.switchUptime || 100) >= parseFloat(exec.slaTarget || 99.3) ? 'MET' : 'BREACH',
        healthScore: s.healthScore,
        healthLabel: s.healthLabel,
    ...
    slide8_RCAPareto: {
      ...
      categories: Array.isArray(rca.breakdown) ? rca.breakdown.slice(0, 8).map((r, idx) => ({
        rca: r.rca,
        count: r.count,
    ...
    slide10_SLA: {
      ...
      siteSlaList: sites.map((s, idx) => ({
        ...
        slaStatus: parseFloat(s.jflSwitchUptime || s.switchUptime || 100) >= parseFloat(exec.slaTarget || 99.3) ? 'MET' : 'BREACH',
        slaCompliance: parseFloat(s.jflSwitchUptime || s.switchUptime || 100) >= parseFloat(exec.slaTarget || 99.3) ? 'MET' : 'BREACH',
        path: `siteSummary[${idx}]`,
      })),
    },
